...
export class MainScene extends Phaser.Scene {
  ...
  private setupInput() {
    ...
    this.keyK.on('down', () => { if (this.sys && this.sys.isActive()) gameStore.toggleSkills(); }); this.keyJ = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.J); this.keyJ.on('down', () => { if (this.sys && this.sys.isActive()) gameStore.toggleQuests(); }); this.keyE.on('down', () => { if (this.sys && this.sys.isActive()) this.doInteract(); }); this.keySpace.on('down', () => { if (this.sys && this.sys.isActive()) this.doAttack(); }); this.keyEsc.on('down', () => { if (this.sys && this.sys.isActive()) gameStore.closeAll(); });
  ...
  private collectChicken(c: ChickenNPC) { const s = gameStore.chickenStates[c.id]; if (!s) return; s.alive = false; s.respawnAt = Date.now() + Phaser.Math.Between(30000, 60000); gameStore.addItem(ITEMS.chicken_meat, 1); if (Math.random() < 0.7) gameStore.addItem(ITEMS.feather, 1); c.collect(); this.time.delayedCall(80, () => { if (c.sprite && c.sprite.active) c.destroy(); }); this.chickens = this.chickens.filter(chi => chi.id !== c.id); playRandomSound(['sfx_pickup_01', 'sfx_pickup_02'], { volume: 0.4 }); gameStore.updateQuestObjective('kill', 'chicken'); gameStore.save(); }

  private collectCrab(c: CrabNPC) { const s = gameStore.crabStates[c.id]; if (!s) return; s.alive = false; s.respawnAt = Date.now() + Phaser.Math.Between(30000, 60000); gameStore.addItem(ITEMS.crab_meat, 1); if (Math.random() < 0.7) gameStore.addItem(ITEMS.crab_shell, 1); c.collect(); this.time.delayedCall(80, () => { if (c.sprite && c.sprite.active) c.destroy(); }); this.crabs = this.crabs.filter(cra => cra.id !== c.id); playRandomSound(['sfx_pickup_02', 'sfx_pickup_03'], { volume: 0.4 }); gameStore.updateQuestObjective('kill', 'crab'); gameStore.save(); }

  private collectBear(b: BearNPC) { const s = gameStore.bearStates[b.id]; if (!s) return; s.alive = false; s.respawnAt = Date.now() + Phaser.Math.Between(60000, 120000); gameStore.addItem(ITEMS.pelt, Phaser.Math.Between(2, 4)); gameStore.addItem(ITEMS.food, 3); b.collect(); this.time.delayedCall(80, () => { if (b.sprite && b.sprite.active) b.destroy(); }); this.bears = this.bears.filter(br => br.id !== b.id); playRandomSound(['sfx_roar_01', 'sfx_roar_02'], { volume: 0.4 }); gameStore.updateQuestObjective('kill', 'bear'); gameStore.save(); }

  private collectRabbit(r: RabbitNPC) { const s = gameStore.rabbitStates[r.id]; if (!s) return; s.alive = false; s.respawnAt = Date.now() + Phaser.Math.Between(30000, 60000); gameStore.addItem(ITEMS.rabbit_meat, 1); gameStore.addItem(ITEMS.pelt, 1); r.collect(); this.time.delayedCall(80, () => { if (r.sprite && r.sprite.active) r.destroy(); }); this.rabbits = this.rabbits.filter(rab => rab.id !== r.id); playRandomSound(['sfx_pickup_01', 'sfx_pickup_03'], { volume: 0.4 }); gameStore.updateQuestObjective('kill', 'rabbit'); gameStore.save(); }

  private harvestResource(res: ResourceObj) { if (!res || !res.active) return; let item, qty = 1; const questUpdates: { type: string; id: string; count: number }[] = []; switch (res.resourceType) { case 'tree': item = ITEMS.wood; qty = Phaser.Math.Between(2, 4); questUpdates.push({ type: 'gather', id: 'wood', count: qty }); break; case 'dead_tree': item = ITEMS.wood; qty = 1; gameStore.addItem(ITEMS.twig, Phaser.Math.Between(2, 4)); questUpdates.push({ type: 'gather', id: 'wood', count: 1 }); break; case 'rock': item = ITEMS.stone; qty = Phaser.Math.Between(1, 3); questUpdates.push({ type: 'gather', id: 'stone', count: qty }); break; case 'small_rock': item = ITEMS.stone; qty = 1; questUpdates.push({ type: 'gather', id: 'stone', count: 1 }); break; case 'bush': item = Math.random() > 0.5 ? ITEMS.fiber : ITEMS.seed; qty = Phaser.Math.Between(1, 2); if (item === ITEMS.fiber) questUpdates.push({ type: 'gather', id: 'fiber', count: qty }); break; case 'iron_ore': item = ITEMS.iron_ore; qty = Phaser.Math.Between(1, 2); questUpdates.push({ type: 'gather', id: 'iron_ore', count: qty }); break; case 'bronze_ore': item = ITEMS.bronze_ore; qty = Phaser.Math.Between(1, 2); break; case 'gold_ore': item = ITEMS.gold_ore; qty = 1; questUpdates.push({ type: 'gather', id: 'gold_ore', count: 1 }); break; } if (item) { gameStore.addItem(item, qty); this.showFloatingText(res.x, res.y - 10, `+${qty} ${item.icon}`, '#ffff00'); playRandomSound(['sfx_pickup_01', 'sfx_pickup_02', 'sfx_pickup_03'], { volume: 0.5 }); } questUpdates.forEach(qu => gameStore.updateQuestObjective(qu.type, qu.id, qu.count)); this.respawnCounter++; gameStore.respawnQueue.push({ x: res.x, y: res.y, type: res.resourceType, hp: res.maxHp, id: `${res.resourceType}_r${this.respawnCounter}_${Date.now()}`, respawnAt: Date.now() + Phaser.Math.Between(60000, 120000) }); this.resources = this.resources.filter(r => r !== res); delete gameStore.resourceStates[res.resourceId]; res.destroy(); gameStore.save(); }
